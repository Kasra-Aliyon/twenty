...
| Connection sends            | Internal API, randomized 30–60 second spacing            | Visible LinkedIn UI, server schedule, workspace-wide cap, local one-minute floor, and configurable 1–40 daily cap | More conservative                                          |
...
- Read requests are serialized at 6–12 second intervals and limited to 8 per
  minute and 60 per hour. An optional daily cap can be configured from 1–200
  requests in Twenty under **Settings → General → Outreach** or in the
  extension popup.
...
- Outbound actions are checked before claim, recorded before the final UI
  action, limited to a user-configurable 1–40 attempts per local day, and
  separated by at least one minute across all sequences.
...
- `npm run compile`
- `npm run build`
- Safety-policy unit tests covering hourly limits, configurable daily limits,
