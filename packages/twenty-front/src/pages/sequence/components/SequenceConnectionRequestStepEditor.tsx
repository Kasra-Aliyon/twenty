...
export const SequenceConnectionRequestStepEditor = ({
  ...
  const isNoteTooLong = noteTemplate.length > LINKEDIN_NOTE_CHARACTER_LIMIT;

  const save = async () => {
    if (isNoteTooLong) {
      enqueueErrorSnackBar({
        message: t`Keep the LinkedIn invitation note within 200 characters.`,
      });
  ...
  return (
    <>
      ...
      <StyledField>
        ...
        />
        <StyledCharacterCount isOverLimit={isNoteTooLong}>
      ...
      <StyledHint>
        {t`This step is always skipped if the contact is already connected or if a connection request is still outstanding.`}
      </StyledHint>
