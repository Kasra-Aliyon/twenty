...
If you host twentyCRM on your own computer, open your browser and navigate to [http://localhost:2000](http://localhost:2000).
...
By default, Docker Compose exposes Twenty on `localhost` at port `2000`. Set
`SERVER_PORT` to change the host port and keep `SERVER_URL` aligned with the
public URL. The container continues to use its internal port `3000`, so reverse
proxies and container health checks should target that internal port.
...
1. **Determine Your Access URL**
    - **Without Reverse Proxy (Direct Access):**
        ...
        ```ini
        SERVER_PORT=2000
        SERVER_URL=http://your-domain-or-ip:2000
        ```
...
2. **Update the `.env` File**
    ...
    Open your `.env` file and update both the host port and public URL:
    ...
    **Examples:**
    - Direct access without SSL:
        ```ini
        SERVER_PORT=2000
        SERVER_URL=http://123.45.67.89:2000
        ```
