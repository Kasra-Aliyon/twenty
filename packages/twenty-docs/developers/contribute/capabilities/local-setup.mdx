...
Twenty's frontend will be running at [http://localhost:2001](http://localhost:2001).
You can log in using the default demo account: `tim@apple.dev` (password: `tim@apple.dev`)

**Backend**

- Twenty's server will be up and running at [http://localhost:2000](http://localhost:2000)
- The GraphQL API can be accessed at [http://localhost:2000/graphql](http://localhost:2000/graphql)
- The REST API can be reached at [http://localhost:2000/rest](http://localhost:2000/rest)

The defaults can be overridden with `REACT_APP_PORT` for the frontend and
`NODE_PORT` for the backend. Keep `REACT_APP_SERVER_BASE_URL`, `FRONTEND_URL`,
and `SERVER_URL` aligned with those overrides. The setup script does not
overwrite existing `.env` files, so older checkouts must update their saved
URLs before restarting. Moving the frontend to a new origin also requires a
one-time sign-in in the browser.
