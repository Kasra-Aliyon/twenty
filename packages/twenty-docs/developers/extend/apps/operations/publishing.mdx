...
|---------|-------|---------|
| `TWENTY_DEPLOY_URL` | `env` in `cd.yml` (defaults to `http://localhost:2000`) | The Twenty server to deploy to. Change this to your real server URL before first use. |
...
<Note>
The default `TWENTY_DEPLOY_URL` of `http://localhost:2000` is a placeholder — it will not reach anything from a GitHub-hosted runner. Update it to your server's public URL (or use a self-hosted runner with network access) before enabling CD.
</Note>
