...
Se ospiti twentyCRM sul tuo computer, apri il browser e naviga a [http://localhost:2000](http://localhost:2000).
...
Per impostazione predefinita, Twenty gira su `localhost` alla porta `2000`. Per accedervi tramite un dominio esterno o indirizzo IP devi configurare il `SERVER_URL` nel tuo file `.env`.
...
1. **Determina il tuo URL di Accesso**
   * **Senza Proxy Inverso (Accesso Diretto):**
     ...
     ```ini
     SERVER_URL=http://tuo-dominio-o-ip:2000
     ```
...
2. **Aggiorna il File `.env`**
   ...
   Apri il tuo file `.env` e aggiorna sia la porta host sia `SERVER_URL`:
