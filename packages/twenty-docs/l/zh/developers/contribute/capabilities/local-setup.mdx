...
Twenty 的前端将在 [http://localhost:2001](http://localhost:2001) 运行。
您可以使用默认的演示帐号登录：`tim@apple.dev` （密码：`tim@apple.dev`）

**后端**

* Twenty 的服务器将在 [http://localhost:2000](http://localhost:2000) 启动并运行
* 可在 [http://localhost:2000/graphql](http://localhost:2000/graphql) 访问 GraphQL API
* 可通过 [http://localhost:2000/rest](http://localhost:2000/rest) 访问 REST API
