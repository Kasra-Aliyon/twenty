...
Twentyのフロントエンドは[http://localhost:2001](http://localhost:2001)で稼働しています。
デフォルトのデモアカウントを使用してログインできます: `tim@apple.dev` (パスワード: `tim@apple.dev`)

**バックエンド**

* Twentyのサーバーは[http://localhost:2000](http://localhost:2000)で稼働しています
* GraphQL API は [http://localhost:2000/graphql](http://localhost:2000/graphql) でアクセスできます
* REST API は [http://localhost:2000/rest](http://localhost:2000/rest) でアクセスできます
