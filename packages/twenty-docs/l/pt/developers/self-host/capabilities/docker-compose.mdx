...
Se você hospedar o twentyCRM no seu próprio computador, abra o navegador e acesse [http://localhost:2000](http://localhost:2000).
...
Por padrão, o Twenty é executado em `localhost` na porta `2000`. Para acessá-lo via um domínio externo ou endereço IP, você precisa configurar o `SERVER_URL` no seu arquivo `.env`.
