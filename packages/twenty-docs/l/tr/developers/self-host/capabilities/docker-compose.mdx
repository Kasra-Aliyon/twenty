...
Eğer twentyCRM'i kendi bilgisayarınızda barındırıyorsanız, tarayıcınızı açın ve [http://localhost:2000](http://localhost:2000) adresine gidin.
...
Varsayılan olarak, Twenty `localhost` üzerinde `2000` portunda çalışır. Harici bir alan adı veya IP adresi aracılığıyla erişim sağlamak için `.env` dosyanızda `SERVER_URL`'i yapılandırmanız gerekir.
...
1. **Erişim URL'nizi Belirleyin**
   * **Ters Proxy Olmadan (Doğrudan Erişim):**
     ...
     ```ini
     SERVER_URL=http://alan-adiniz-veya-ip:2000
     ```
...
2. **`.env` Dosyasını Güncelleyin**
   ...
   `.env` dosyanızı açın ve hem ana makine portunu hem de `SERVER_URL`'i güncelleyin:
