...
Pokud hostujete twentyCRM na svém vlastním počítači, otevřete svůj prohlížeč a přejděte na stránku [http://localhost:2000](http://localhost:2000).
...
Ve výchozím nastavení běží Twenty na `localhost` na portu `2000`. Pro přístup přes externí doménu nebo IP adresu musíte nakonfigurovat `SERVER_URL` ve vašem `.env` souboru.
...
1. **Určete svou přístupovou URL**
   * **Bez Reverzní Proxy (Přímý Přístup):**
     ...
     ```ini
     SERVER_URL=http://vaše-doména-nebo-ip:2000
     ```
...
2. **Aktualizujte `.env` Soubor**
   ...
   Otevřete svůj `.env` soubor a aktualizujte hostitelský port i `SERVER_URL`:
