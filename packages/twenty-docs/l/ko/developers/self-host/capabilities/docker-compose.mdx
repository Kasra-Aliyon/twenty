...
twentyCRM을 개인 컴퓨터에 호스팅하는 경우 브라우저를 열고 [http://localhost:2000](http://localhost:2000)으로 이동하세요.
...
기본적으로 Twenty는 `localhost`의 포트 `2000`에서 실행됩니다. 외부 도메인 또는 IP 주소로 액세스하려면 `.env` 파일의 `SERVER_URL`을 구성해야 합니다.
